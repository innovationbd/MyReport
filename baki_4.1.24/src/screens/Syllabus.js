...
import { Button,StyleSheet,View,Text,ScrollView,Alert, Modal, FlatList, TouchableOpacity } from 'react-native';
  ...
  const filterData = (item) =>{
    if(userInput === ""){
      return (
        ...
        onPress={(e) => bookRead.includes(item.id) ? setbookRead(bookRead.filter(b => b !== item.id)) : setbookRead(old => [...old, item.id])}
